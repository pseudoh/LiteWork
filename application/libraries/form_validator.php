...
/**
** A form validation library that offers validation and filtering on form data through a clear interface.
** In addition it handles errors which can be easily accessed.
** 
...
** strip_tags : Parse the value out of html elements and disregard the rest
** raw_text : Encodes all html tags
** escape : Escape all string for mysql inject prevention
**
	...
	private function process_rules($field, $label, $rules) {
		...
		foreach ($rules as $rule) {
			...
			switch ($rule_name) {
				...
				break;
				case 'strip_tags': //Filter: Strips html text
					$request_value = strip_tags($request_value);
				break;
				case 'raw_text': //Filter: Encode html characters
					$request_value = htmlspecialchars($request_value);
				break;
				case 'escape': //Filter: Escape all string and make them query friendly
					$request_value = mysql_real_escape_string($request_value);
				break;
			}
